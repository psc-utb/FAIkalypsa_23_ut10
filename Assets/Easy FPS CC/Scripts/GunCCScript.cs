...
public class GunCCScript : MonoBehaviour {
	...
	private PlayerMovementCCScript pmCCS;
	...
	void Awake(){
		...
		mls = GameObject.FindGameObjectWithTag("Player").GetComponent<MouseLookCCScript>();
		...
		pmCCS = player.GetComponent<PlayerMovementCCScript>();
	...
	*/
	...
	void CrossHairExpansionWhenWalking(){
		...
		if(player.GetComponent<CharacterController>().velocity.magnitude > 1 && Input.GetAxis("Fire1") == 0){//ifnot shooting

			expandValues_crosshair += new Vector2(20, 40) * Time.deltaTime;
			if(player.GetComponent<PlayerMovementCCScript>().MaxMoveSpeed < runningSpeed){ //not running
	...
	void Sprint(){// Running();  so i can find it with CTRL + F
		if (Input.GetAxis ("Vertical") > 0 && Input.GetAxisRaw ("Fire2") == 0 && meeleAttack == false && Input.GetAxisRaw ("Fire1") == 0) {
			if (Input.GetKeyDown (KeyCode.LeftShift)) {
				if (pmCCS.MaxMoveSpeed == walkingSpeed) {
					pmCCS.MaxMoveSpeed = runningSpeed;//sets player movement peed to max

				} else {
					pmCCS.MaxMoveSpeed = walkingSpeed;
				}
			}
		} else {
			pmCCS.MaxMoveSpeed = walkingSpeed;
	...
	*/
	...
	void PositionGun(){
		...
		pmCCS.CameraPosition = new Vector3(currentRecoilXPos,currentRecoilYPos, 0);
	...
	*/
	...
	private void ShootMethod(){
		if(waitTillNextFire <= 0 && !reloading && pmCCS.MaxMoveSpeed < 5){

			if(bulletsInTheGun > 0){

				int randomNumberForMuzzelFlash = muzzelFlash.Length > 1 ? Random.Range(0,muzzelFlash.Length) : muzzelFlash.Length-1;
				if (bullet)
	...
	*/
	...
	IEnumerator Reload_Animation(){
		if(bulletsIHave > 0 && bulletsInTheGun < amountOfBulletsPerLoad && !reloading/* && !aiming*/){
			...
			if (meeleAttack == false && pmCCS.MaxMoveSpeed != runningSpeed) {
				//print ("tu sam");
				if (player.GetComponent<PlayerMovementCCScript> ()._freakingZombiesSound)
					player.GetComponent<PlayerMovementCCScript> ()._freakingZombiesSound.Play ();
				else
	...
	*/
	void Animations(){
		...
		if(handsAnimator){
			...
			handsAnimator.SetFloat("walkSpeed",pmCCS.CurrentSpeed);
			handsAnimator.SetBool("aiming", Input.GetButton("Fire2"));
			handsAnimator.SetInteger("maxSpeed", pmCCS.MaxMoveSpeed);
			if(Input.GetKeyDown(KeyCode.R) && pmCCS.MaxMoveSpeed < 5 && !reloading && !meeleAttack/* && !aiming*/){
				StartCoroutine("Reload_Animation");
			}
		}
