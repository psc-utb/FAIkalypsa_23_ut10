...
public class MouseLookCCScript : MonoBehaviour {
	...
	*/
	void  Update(){
		...
		if(GetComponent<PlayerMovementCCScript>().CurrentSpeed > 1)
			HeadMovement ();
...
void FixedUpdate(){
	...
	else if(GetComponent<PlayerMovementCCScript>().MaxMoveSpeed > 5){
...
void WeaponRotation(){
	if(!weapon){
		...
		if(weapon){
			if(weapon.GetComponent<GunCCScript>()){
				try{
					gun = GameObject.FindGameObjectWithTag("Weapon").GetComponent<GunCCScript>();
